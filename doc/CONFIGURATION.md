...
| history_postgresql_user | | String | No | Role used to connect to the history database (used by the `postgresql` backend). Required when `history_backend` is `postgresql` and `history` is set. Changes require restart. |
